...
public class NumberFormatterDataBindingSnippet {
	static class Person {
        ...
        public void setPerc(double perc) {
            propertyChangeSupport.firePropertyChange("perc", this.perc, this.perc = perc);
        }
    ...
    new Label(shell, SWT.NONE).setText("Perc.:");
    final FormattedText percentField = new FormattedText(shell, SWT.BORDER | SWT.RIGHT);
    percentField.setFormatter(new PercentFormatter());
    GridDataFactory.swtDefaults().hint(100, SWT.DEFAULT).applyTo(percentField.getControl());
    ...
    modelButton.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				model.setText(person.getName() + " - " + person.getAge()+ " - " + person.getPerc());
			}
    });
    ...
    context.bindValue(new FormattedTextObservableValue(percentField, SWT.Modify), BeansObservables.observeValue(person, "perc"));
