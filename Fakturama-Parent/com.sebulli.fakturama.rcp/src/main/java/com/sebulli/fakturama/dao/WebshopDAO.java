...
//		query.setHint(QueryHints.FETCH, "FKT_WEBSHOP." + WebShop_.stateMapping.getName()/* + "." + WebshopStateMapping_.name.getName()*/);
	...
	public WebShop clearOldMappings(String webshopId) throws FakturamaStoringException {
		...
		if (webShop != null) {
			List<Long> orphanedStateIds = webShop.getStateMapping().stream().map(WebshopStateMapping::getId)
					.collect(Collectors.toList());
			webShop.setStateMapping(new ArrayList<>());

			// remove old mapping entries from database
			CriteriaBuilder criteriaBuilder = getEntityManager().getCriteriaBuilder();
			CriteriaDelete<WebshopStateMapping> deleteQuery = criteriaBuilder
					.createCriteriaDelete(WebshopStateMapping.class);
			...
			try {
				...
				throw new FakturamaStoringException(
						"Error cleaning web shop state mappings from database for your web shop (name=" + webshopId
								+ ").",
						e);
			}
