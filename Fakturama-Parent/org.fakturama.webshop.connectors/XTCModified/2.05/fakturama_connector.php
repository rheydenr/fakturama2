 ...
 */
define ('FAKTURAMA_CONNECTOR_VERSION','2.05'); 
/* 
 * Date: 2019-11-30
 * 
...
// 'XTCMODIFIED'	// modified Shop 1.06			www.modified-shop.org
...
require_once (DIR_WS_INCLUDES.'version.php'); 
  ...
  require_once(DIR_FS_INC . 'db_functions_mysqli.inc.php');
...
//  require_once (DIR_FS_CATALOG.DIR_WS_CLASSES.'Phpmailer.php');
...
if (strncmp($action, "get", 3) == 0) {
  ...
  $action_getstatusvalues = strpos($action,"status");
}


// generate header of response
echo ("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n");
echo ("<webshopexport version=\"". FAKTURAMA_CONNECTOR_VERSION . "\" xmlns=\"http://www.fakturama.org\">\n");
...
echo ("<complete_version>" . PROJECT_VERSION . "</complete_version>");
		...
		if ($action_getorders){
			...
			while ($check_orders = sbf_db_fetch_array($check_orders_query)) {
				...
				while ($orders_history = sbf_db_fetch_array($orders_history_query)) {
					...
					//  add a CDATA section in case of special signs in comment like < or >
					$comment = "<![CDATA[". sbf_db_output($orders_history['comments']) ."]]>";
			
						echo ("    <comment date=\"" . date_to_ISO($orders_history['date_added']) . "\">");
						echo ( my_encode(nl2br($comment)));
						echo ("</comment>\n");
					}
				}
